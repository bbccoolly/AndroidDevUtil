...
object UploadServiceConfig {
     ...
     */
    const val defaultUserAgent = "okhttp/4.10.0"//"AndroidUploadService/4.8.0"
    ...
    var notificationConfigFactory: (context: Context, uploadId: String) -> UploadNotificationConfig = { context, uploadId ->
        ...
        UploadNotificationConfig(
            ...
            progress = UploadNotificationStatusConfig(
                title = title,
                message = "上传进度 ${Placeholder.UploadRate} (${Placeholder.Progress})",
                actions = arrayListOf(
            ...
            success = UploadNotificationStatusConfig(
                title = title,
                message = "上传在 ${Placeholder.ElapsedTime} 内成功完成"
            ),
...
//    var httpStack: HttpStack = HurlStack()
    var httpStack: HttpStack = OkHttpStack()
